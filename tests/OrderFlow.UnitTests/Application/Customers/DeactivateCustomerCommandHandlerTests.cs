...
public sealed class DeactivateCustomerCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_ActiveCustomer_TransitionsToInactive()
    {
        var customer = ActiveCustomer();
        var handler = new DeactivateCustomerCommandHandler(new FakeCustomerRepository(customer));

        var result = await handler.Handle(new DeactivateCustomerCommand(customer.Id));

        Assert.Equal(nameof(CustomerStatus.Inactive), result.Status);
        Assert.Equal(CustomerStatus.Inactive, customer.Status);
    }

    [Fact]
    public async Task Handle_AlreadyInactiveCustomer_IsIdempotent()
    {
        var customer = ActiveCustomer();
        customer.Deactivate();
        var handler = new DeactivateCustomerCommandHandler(new FakeCustomerRepository(customer));
    ...
    {
        var handler = new DeactivateCustomerCommandHandler(new FakeCustomerRepository(customer: null));

        await Assert.ThrowsAsync<NotFoundException>(() => handler.Handle(new DeactivateCustomerCommand(999)));
    }
    ...
    public async Task Handle_InvalidCustomerId_ThrowsValidation()
    {
        ...
        await Assert.ThrowsAsync<ValidationException>(() => handler.Handle(new DeactivateCustomerCommand(0)));
    }
    ...
    private sealed class FakeCustomerRepository : ICustomerRepository
    {
        ...
        public Task<bool> ExistsAsync(int customerId, CancellationToken cancellationToken = default)
            => throw new NotSupportedException("Not needed for DeactivateCustomer tests.");
